...
async def _read_upload(file: UploadFile) -> bytes:
    """Read only up to the configured limit plus one byte to enforce upload size."""
...
async def upload_document(
    agent_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)
):
    """Upload a validated document into the selected agent's private storage."""
    _require_agent(db, agent_id)
    try:
        return document_service.store_document(
            db, agent_id=agent_id, filename=file.filename, content=await _read_upload(file)
        )
    except document_service.DocumentValidationError as error:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail=str(error)) from error
    except document_service.DuplicateDocumentError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
    except document_service.DocumentStorageError as error:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Document storage failed") from error


@router.get("", response_model=list[KnowledgeDocumentRead])
def list_documents(agent_id: str, db: Session = Depends(get_db)):
    """List document metadata for one agent."""
    _require_agent(db, agent_id)
    return document_service.list_agent_documents(db, agent_id)
...
def delete_document(
    ...
    """Delete an agent-owned document and its safely stored file."""
    _require_agent(db, agent_id)
    document = document_service.get_agent_document(db, agent_id, document_id)
    if document is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Document not found")
    try:
        vector_store.delete_document_vectors(agent_id, document.id)
        document_service.delete_agent_document(db, document)
    except document_service.DocumentStorageError as error:
